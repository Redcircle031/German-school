...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const titles = { pl: 'Ferie 2026/27 | WBS', de: 'Ferien 2026/27 | WBS', en: 'Holidays 2026/27 | WBS' };
  return { title: titles[locale as keyof typeof titles] || titles.en };
}

const translations = {
  pl: { title: 'Kalendarz Ferii 2026/27', subtitle: 'Plan ferii i dni wolnych w roku szkolnym 2026/2027', semesterLabel: 'Semestr', downloadLabel: 'Pobierz PDF' },
  de: { title: 'Ferienplan 2026/27', subtitle: 'Ferien- und unterrichtsfreie Tage im Schuljahr 2026/2027', semesterLabel: 'Halbjahr', downloadLabel: 'PDF herunterladen' },
  en: { title: 'Holiday Calendar 2026/27', subtitle: 'Holidays and days off in the 2026/2027 school year', semesterLabel: 'Semester', downloadLabel: 'Download PDF' },
};

export default async function Holidays2026Page({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = translations[locale as keyof typeof translations] || translations.en;
  const yearData = holidaysData.schoolYears['2026-27'];
    ...
    <>
      ...
      <section className="section bg-white">
        <div className="container-custom max-w-3xl">
          <div className="mb-8">
            <a href="/pdfs/ferienplan_2026_27.pdf" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-red-600 px-6 py-3 font-semibold text-white hover:bg-red-700">
