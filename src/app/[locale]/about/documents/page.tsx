...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const titles = { pl: 'Dokumenty | WBS', de: 'Dokumente | WBS', en: 'Documents | WBS' };
  return { title: titles[locale as keyof typeof titles] || titles.en };
}

const translations = {
  pl: { title: 'Dokumenty Szkolne', subtitle: 'Oficjalne regulaminy, formularze i programy' },
  de: { title: 'Schuldokumente', subtitle: 'Offizielle Ordnungen, Formulare und Programme' },
  en: { title: 'School Documents', subtitle: 'Official regulations, forms, and programmes' },
};
...
const categoryIcons: Record<string, typeof FileText> = {
  forms: FileText, regulations: Shield, schedules: Calendar, programs: BookOpen, other: Settings,
};

function cleanTitle(filename: string): string {
  return filename.replace(/\.pdf$/, '').replace(/_/g, ' ').replace(/%[0-9A-Fa-f]{2}/g, m => decodeURIComponent(m)).replace(/^\d+\.\s*/, '').replace(/\s+/g, ' ').trim();
}
...
export default async function DocumentsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const categoryOrder = ['regulations', 'forms', 'programs', 'schedules', 'other'];
            ...
            return (
              <div key={cat}>
                ...
                <div className="grid gap-3">
                  {pdfs.map((pdf) => (
                    <a key={pdf.filename} href={pdf.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between rounded-xl bg-neutral-50 p-4 transition-colors hover:bg-red-50">
                      <div className="flex items-center gap-4">
                        <div className="flex size-10 items-center justify-center rounded-lg bg-red-100"><FileText className="size-5 text-red-600" /></div>
                        <div>
